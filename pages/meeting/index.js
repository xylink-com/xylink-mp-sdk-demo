...
const defaultAvatar = 'https://cdn.xylink.com/miniProgram/photo_default.png';
...
Page({
  data: {
    // 是否开启麦克风，false代表开启，true代表关闭，支持动态设置
    muted: false,
    // 是否开启摄像头，true代表开启摄像头，false代表关闭摄像头，支持动态设置
    camera: true,
    // 切换前后置摄像头，不支持动态设置，需要调用SDK接口实现；
  ...
  onRoomEvent(ms) {
    ...
    switch (type) {
      ...
      case 'roster':
        ...
        const isMyself =
          (roster.length === 1 && fristCallNumber === this.callNumber) ||
          roster.length === 0;
